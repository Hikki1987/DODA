...
| OD-002 | Birinchi real konnektor: Google Calendar, Gmail, Telegram yoki boshqa? | S5 oxiri | S6, S7 | **Resolved — Telegram.** Product Owner qarori. Bu ADR-007'ni "Open"dan "Accepted"ga o'tkazadi (pastga qarang) va FR-ACT-001/xavfsizlik ko'rib chiqishda topilgan `tool_name → minimal risk_level` server-side siyosat qatlamini haqiqiy zaruratga aylantiradi — endi "kelajakda kerak bo'ladi" emas, aniq maqsadli tool nomi (`telegram.send_message`) bilan bog'lab qurilishi kerak edi (qurildi, pastdagi CLAUDE.md yozuviga qarang). Haqiqiy Telegram Bot API integratsiyasi (bot token, broker orqali qisqa muddatli credential, real xabar yuborish) hali qurilmagan — bu haqiqiy bot token'ni talab qiladi, uni Product Owner xavfsiz kanal orqali (hech qachon chat/prompt matniga yozib emas) taqdim etishi kerak. |
